...
export function createSignalAdapter(config: {
  ...
  async function handleEnvelope(envelope: SignalEnvelope): Promise<void> {
    ...
    // .pdf, archives. Upstream accepts images only, so a markdown file attached
    // to a message was discarded with no warning and no log line — the message
    // arrived looking like plain text.
    ...
    // Inline images and documents as base64 so the host's inbox writer
    // (`session-manager.ts`) materializes them under the session's
    // `/workspace/inbox/<message-id>/` directory.
    //
    // Upstream pushes `{ path: <host-path>, ... }` instead, but the signal-cli
    // attachments directory is not mounted into the container, and the inbox
    // writer only materializes attachments carrying inline `data` — so the
    // agent received a path that didn't resolve. Do not "simplify" this back
    // to a path reference without first mounting that directory.
