...
export class SignalChannel implements Channel {
  ...
  private async processImageAttachment(
    ...
    if (!existsSync(srcPath)) {
      logger.warn({ attId: att.id }, 'Signal: image attachment file not found');
      return null;
    }
    const buffer = await readFile(srcPath);
    const groupDir = resolveGroupFolderPath(groupFolder);
    await mkdir(path.join(groupDir, 'attachments'), { recursive: true });
  ...
  private async handleSseEvent(event: SseEvent): Promise<void> {
        ...
        if (syncSent.attachments?.length) {
          for (const att of syncSent.attachments) {
            if (att.contentType?.startsWith('image/')) {
              try {
                const ref = await this.processImageAttachment(
                  att,
                  group.folder,
                );
                if (ref) {
                ...
                logger.warn(
                  { err, attId: att.id },
                  'Signal: failed to process image attachment',
                );
              }
            }
          }
    ...
    if (dataMessage.attachments?.length) {
      for (const att of dataMessage.attachments) {
        if (att.contentType?.startsWith('image/')) {
          try {
            const ref = await this.processImageAttachment(att, group.folder);
            if (ref) {
