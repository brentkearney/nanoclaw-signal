...
export class SignalChannel implements Channel {
  ...
  constructor(
    ...
    private signalDataDir: string = path.join(
      os.homedir(),
      '.local',
      'share',
      'signal-cli',
  ...
  private async copyPdfAttachment(
    att: NonNullable<SignalDataMessage['attachments']>[0],
    groupFolder: string,
  ): Promise<string | null> {
    if (!att.id) return null;
    const srcPath = path.join(this.signalDataDir, 'attachments', att.id);
    if (!existsSync(srcPath)) {
      logger.warn({ attId: att.id }, 'Signal: PDF attachment file not found');
      return null;
    }
    const groupDir = resolveGroupFolderPath(groupFolder);
    const attachDir = path.join(groupDir, 'attachments');
    await mkdir(attachDir, { recursive: true });
    const filename = path.basename(att.filename || `doc-${Date.now()}.pdf`);
  ...
  private async handleSseEvent(event: SseEvent): Promise<void> {
    ...
    if (syncSent) {
      ...
      if (dest === this.account) {
        ...
        if (!text && !(syncSent.attachments?.length)) return;
        ...
        if (syncSent.attachments?.length) {
          for (const att of syncSent.attachments) {
            if (att.contentType === 'application/pdf') {
              try {
                const ref = await this.copyPdfAttachment(att, group.folder);
                if (ref) {
                ...
                logger.warn(
                  { err, attId: att.id },
                  'Signal: failed to copy PDF attachment',
                );
              }
            }
          }
    ...
    if (!text && !(dataMessage.attachments?.length)) return;
